...
%% steepest descent
    ...
    xx=xx(:);
    pathxbar=fft(xx,N);
    xbar=log(pathxbar(1:N/2).*conj(pathxbar(1:N/2))/N);
    k = [0:N/2 (-N/2+1):-1] ;
    f = k./(2*dx);
    freqt=log(f(1:N/2));
    freqt(freqt<0)=0;
    ...
    ybar=log(pathybar(1:N/2).*conj(pathybar(1:N/2))/N);
    ...
    zbar=log(pathzbar(1:N/2).*conj(pathzbar(1:N/2))/N);

    k = [0:N/2 (-N/2+1):-1] ;
    f = k./(2*dx);
    freqt=log(f(1:N/2));
    freqt(freqt<0)=0;
    Bz= bin(freqt, zbar, 30);
    fitz=robustfit(Bz(:,1), Bz(:,8));
    Spectral_slopez = abs(fitz(2));
...
save('fft_topo_profile_descent_curv.mat', 'beta','topo2D','transect', 'descentx', 'descenty', 'descentz','curv')
